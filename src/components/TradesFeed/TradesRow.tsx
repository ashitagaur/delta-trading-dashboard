...
export const TradesRow = React.memo(({ trade, symbol }: Props) => {
  ...
  const timeDisplay = `${time.getHours().toString().padStart(2, '0')}:${time.getMinutes().toString().padStart(2, '0')}:${time.getSeconds().toString().padStart(2, '0')}`;
  ...
  return (
    <div className={`flex items-center justify-between py-[3px] px-2 text-[11px] font-mono hover:bg-gray-800/50 transition-colors ${borderClass}`}>
      <span className={`w-1/3 text-left ${colorClass}`}>{priceDisplay}</span>
      <span className="w-1/3 text-center text-gray-200">{finalSizeDisplay}</span>
      <span className="w-1/3 text-right text-gray-500">{timeDisplay}</span>
    </div>
  );
});
